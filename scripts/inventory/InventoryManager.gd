...
func add(item_id: String, count: int = 1) -> bool:
    ...
    if not DataRegistry.has_id("items", item_id):
        ...
        return false
    if not ItemStacking.add(GameState.player["inventory"], item_id, count):
        return false
...
func remove(item_id: String, count: int = 1) -> bool:
    ...
    if not DataRegistry.has_id("items", item_id):
        ...
        return false
    if not ItemStacking.remove(GameState.player["inventory"], item_id, count):
        return false
...
func get_count(item_id: String) -> int:
    return ItemStacking.count_of(GameState.player["inventory"], item_id)
